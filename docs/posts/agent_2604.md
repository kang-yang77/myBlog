...
Tips：在沙盒环境中执行，即使大模型产生了有问题的代码，数据丢失和敏感信息泄漏的风险也会更低。docker E2B

#### MCP(模型上下文协议)

##### 背景/解决的问题
![](figs/p10.png)
统一减少社区总工作量，为LLM提供更多上下文和获取数据以及支持工具调用

##### 架构
MCP 采用了标准的客户端-服务端解耦架构
1. MCP Host
	运行大模型和发起调用的应用程序（例如 Claude Desktop，Cursor)
2. MCP Server
	运行在 Host 内部，负责与服务器维持双向通信通道
3. MCP client
	每个 Server 专门负责连接并包装一个特定的底层资源（ MySQL、github、MQ 等）。
...
### 如何构建Agent？
	在开发Agent时，很难提前判断系统在哪些地方能正常运行，哪些地方表现不佳
	先尝试快速搭建一个粗糙的系统是常见的选择，这样就可以实际测试，完善这个系统 
...
#### 误差分析与后续优化
 ...
 Tips：
 - 养成查看追踪链路（Traces）的习惯，检查工作流程中的每一步，每一步之后的中间输出结果
    
- 执行错误分析，定位导致最终输出效果不佳的具体组件。
    
- 根据错误分析的结果，决定后续优化工作的发力点。
